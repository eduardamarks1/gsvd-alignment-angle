...
function plot_instance_panel(v, coeff_A, coeff_B, sample_digit, digitA, digitB)
    ...
    p_coeffs = plot(
        1:length(coeff_A),
        coeff_A;
        label="x via digito $digitA, ||x|| = $(round(nA, digits=2))",
        color=:steelblue,
        linewidth=1.5,
        xlabel="coordenada",
    ...
    plot!(
        ...
        1:length(coeff_B),
        coeff_B;
        label="y via digito $digitB, ||y|| = $(round(nB, digits=2))",
        color=:coral,
        linewidth=1.5
    )
    hline!(p_coeffs, [0.0]; color=:gray, linestyle=:dash, linewidth=1, label="")

    return plot(p_img, p_coeffs; layout=(1, 2), size=(980, 330))
end
function run_coeff_histogram_experiment(; digitA=4, digitB=9, n=900, m=900,
                                         ...
                                         savepath="coeff_linhas_sobrepostas_4_9.png")
